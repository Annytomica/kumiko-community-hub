...
class ArticleCommentForm(forms.ModelForm):
    ...
    **Widgets**
    `body`: Configured as a `Textarea` widget with placeholder text:
...
class ArticleLikeForm(forms.ModelForm):
    ...
    **Fields**
    ``like``
        A BooleanField that represents whether the user
        likes the article or not.

    **Widgets**
    `like`: Configured as a `HiddenInput` widget,
    meaning this field will not be visible to users.
    This allows toggling of the like status via button
    without needing direct form input from the user.

    **Usage**
    This form is used in the article detail view (`single_article`) to handle
    the like button
