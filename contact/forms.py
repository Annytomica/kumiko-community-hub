...
class ContactMessage(forms.ModelForm):
    ...
    **Model**
    Uses :model:`contact.ContactMessage` to create and save
    ...
    **Widgets**
    ...
    `message`: Configured as a `Textarea` widget with a placeholder
        "Your message here" and 6 rows.
        Defining 6 rows creates enough space for users to write messages
        The placeholder helps users identify the field for entering message

    **Usage**
    This form is used on the contact page to collect messages or inquiries
    from visitors.
    The widgets enhance user experience by providing visual cues (placeholders)
    and appropriate field sizes, making it easier for users to understand what
